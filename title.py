...
class Title(C.Structure):
    ...
    name: Annotated[str, CStrField(9)]  # max length w/o nulls seems to be 7

    # exp_bonus and bor_bonus should be multiplied by 5% to find the actual
    # percentage. So the max is 0x7f * 5% = 635%.
    exp_bonus: Annotated[int, CField(C.c_int8)]
    bor_bonus: Annotated[int, CField(C.c_int8)]

    # The changes to the palette when this title is applied to a character.
    # (Unsure of the exact format.)
    color_change: Annotated[int, CField(C.c_uint8)]

    # Obtainable titles are ranks 0-7, but there are some unobtainable rank 8s.
    rank: Annotated[int, CField(C.c_uint8)]

    rarity: Annotated[Rarity, CField(C.c_uint8)]

    # These are encoded strangely; see the elemental_resistances getter for
    # details.
    _elemental_resistances: Annotated[int, CField(C.c_uint8)]

    guard: Annotated[int, CField(C.c_uint8)]
    steal: Annotated[int, CField(C.c_uint8)]
    move: Annotated[int, CField(C.c_uint8)]
    ...
    stat_bonuses: Annotated[FixedLengthMutableSequence[int], CField(C.c_uint8 * 6)]
    ...
    sp_bonuses: Annotated[FixedLengthMutableSequence[int], CField(C.c_int8 * 7)]

    _zero: Annotated[Sequence[int], CField(C.c_uint8 * 3)]

    # Skills conferred by this title. List is terminated by a zero.
    skill_ids: Annotated[FixedLengthMutableSequence[int], CField(C.c_uint16 * 4)]
